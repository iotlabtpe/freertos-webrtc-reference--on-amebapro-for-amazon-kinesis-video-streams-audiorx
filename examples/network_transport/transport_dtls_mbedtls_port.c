...
void mbedtls_timing_set_delay(void *data, uint32_t int_ms, uint32_t fin_ms) {
    LogDebug(("mbedtls_timing_set_delay: int_ms %i,fin_ms %i", int_ms, fin_ms)); ;
    ...
    LogDebug(("mbedtls_timing_set_delay start_ticks: %i",ctx->start_ticks));
}
...
int mbedtls_timing_get_delay(void *data) {
    ...
    LogDebug(("mbedtls_timing_get_delay elapsed_ticks: %i",elapsed_ticks));
    if (ctx->fin_ms == 0) {
        return -1;
    }
